...
# Použijeme doporučený stabilní model (Claude 3.5 Sonnet)
...
def enrich_product_with_ai(clean_name):
    if not client:
        # Ukázkový režim, pokud na webu ještě není nastavený API klíč
        return {
...
if not client:
    st.info("ℹ️ **Aplikace nyní běží v ukázkovém režimu.** Jakmile do Streamlit Cloud přidáte svůj ANTHROPIC_API_KEY, začne živě volat Claude AI.")
...
if st.button("Spustit AI transformaci", type="primary"):
    ...
    if not raw_products:
        ...
        if results:
            ...
            # Export do formátu JSON pro stažení
            output_data = {
